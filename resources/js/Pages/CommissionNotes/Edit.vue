...
const form = useForm({
    ...
    branch_id:   props.note.branch_id,
    employee_id: props.note.employee_id,
    description: props.note.description,
    date       : props.note.date,
    amount:      props.note.amount,
})

function route(name: string, params?: any, absolute: boolean = true) {
    return ziggyRoute(name, params, absolute, page.props.ziggy)
}

const selectedCompany = ref(String(props.note.company_id))
const selectedBranch  = ref(String(props.note.branch_id))
...
function submit() {
    form.put(route('commission-notes.update', props.note.id), {
        preserveScroll: true,
    })
}
    ...
    <AppLayout>
        <div class="max-w-xl">
            <div class="flex items-center gap-4 mb-6">
                ...
                <h1 class="text-xl font-bold text-gray-800">Edit Commission Note</h1>
            </div>
            ...
            <form @submit.prevent="submit" class="bg-white rounded-lg shadow p-6 space-y-4">
                ...
                <div>
                    ...
                    >
                        ...
                        <option v-for="b in availableBranches" :key="b.id" :value="String(b.id)">{{ b.name }}</option>
                    </select>
